...
HC::Coordinates::Coordinates(const HControl::Coordinates::Point pt) noexcept
	:point(pt) {}
...
void HC::Coordinates::append(const std::array<unsigned int, 2> &coord, hc_axis_arr const &plot_values) {
	coordinates = coord;
    const auto update_value = hc_delta_x_y_z(plot_values, axis);
	update_to_new_position = update_value >= hc_change_folds;
	// Update only if the calculated difference is >= 7x folds
	// Although this value is a development approximation, therefore, it can change to something else
